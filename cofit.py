...
with aqui_params[1]:
  purchase_price = 1e6 * st.slider("Purchase Prince ($)", min_value=0.5, max_value=5.0, value=1.4, format="%0.01fM")
with aqui_params[2]:
  transfer_fee_percent = st.slider("NJ Realtor Transfer Fee (%)", min_value=0.0, max_value=5.0, value=1.0, step=0.1, format="%0.1f")
with aqui_params[3]:
...
with aqui_results[3]:
  st.metric("NJ Realtor Transfer Fee ($)", show_value(transfer_cost))
with aqui_results[2]:
